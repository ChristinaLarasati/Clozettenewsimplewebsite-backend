        ...
        <div class="post-preview">        
            <div class="card mb-4" v-for="post in posts" :key="post.id">
                <!-- <img class="card-img-top" :src="'/post_image/' + post.image" alt="Est nostrum sunt." title="Est nostrum sunt.">
                 -->
                 <!-- <img class="card-img-top" src="https://cimgr.clozette.co.id/indo/f_jpg,w_300,c_fit/uploads/da25bfedc80c40dfa8e78afc8e19616b/2ad575e310464a7285b577acf2e0e7af.png"  > -->
                <div class="categories" style="margin: 10px 0 0 0;">
                 <small style="margin-top: 20px;border-radius: 5px;border: 1px solid gray;padding: 5px;" v-for="category in post.categories" :key="category">                
                <router-link :to="{ path: '/post/category/' + category.slug }" append style="color:black;  text-decoration: none;">{{ category.name }}</router-link>
                </small>            
                    </div>
                     <div class="card-body">
            ...
            <ul v-if="pagesNumber.length > 1 && posts.length > 0" class="pagination justify-content-center mb-4">
                <ul class="pagination">
                    ...
                    <li v-for="page in pagesNumber" :key="page.id" :class="{'active page-item': page == pagination.current_page, 'page-item' : page !== pagination.current_page }" v-on:click.prevent="changePage(page)">
...
<script>
    export default {
        ...
        methods: {
            ...
            read() {
                var url = `get-posts`;
                if (typeof this.$route.params.tutorilas !== 'undefined') {
                    url = `get-category-posts/tutorials${this.$route.params.tutorilas}`;
